...
const TrainList: React.FC<TrainListProps> = ({ searchParams, onBack, onPurchaseSuccess }) => {
  ...
  const isRoundTrip = searchParams.searchType === 'roundTrip';
  const isTransfer = searchParams.searchType === 'transfer';
  ...
  const fetchTickets = async () => {
    ...
    try {
      if (isRoundTrip) {
        // Fetch both outbound and return tickets in parallel
        ...
        setTickets(outboundData);
        setReturnTickets(returnData);
      } else {
  ...
  const getMinPrice = (ticket: TrainTicket): number => {
    if (ticket.prices) {
      const prices = Object.values(ticket.prices).filter((p): p is number => p !== undefined && p > 0);
      return prices.length > 0 ? Math.min(...prices) : ticket.price;
    }
    return ticket.price;
  };
        ...
        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center justify-between md:flex-col md:items-end gap-2 md:min-w-[120px]">
            <div className="text-right">
              ...
              <span className="text-3xl font-bold text-orange-500">{minPrice}</span>
            </div>
  ...
  // Render ticket list section
  const renderTicketList = (ticketList: TrainTicket[], title: string, date: string, isReturn?: boolean) => (
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2 mb-4">
        {isReturn && <ArrowRightLeft className="h-4 w-4 text-blue-500 rotate-180" />}
        <h3 className="font-bold text-gray-800">
          {isReturn ? `${searchParams.to} → ${searchParams.from}` : title}
        </h3>
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <h2 className="text-2xl font-bold text-gray-800">
              ...
              {isRoundTrip && (
                <>
                  ...
                  <span className="text-blue-600">往返</span>
                </>
              )}
              {isTransfer && (
                <>
                  <span className="mx-2 text-gray-300">|</span>
                  <span className="text-blue-600">中转</span>
                </>
              )}
            </h2>
      ...
      {loading ? (
        <div className="space-y-4">
          {Array.from({ length: isRoundTrip ? 6 : 3 }).map((_, i) => (
        ...
        /* Round-trip: Side by side layout */
        <div className="flex gap-6">
          {renderTicketList(filteredTickets, `去程: ${searchParams.from} → ${searchParams.to}`, searchParams.date)}
          <div className="w-px bg-gray-200 flex-shrink-0"></div>
          {renderTicketList(returnTickets, `返程: ${searchParams.to} → ${searchParams.from}`, searchParams.returnDate || searchParams.date, true)}
        </div>
