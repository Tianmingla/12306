...
def save_to_database(
    ...
    try:
        ...
        if carriage_records:
            ...
            logger.debug(f"已插入 {len(carriage_records)} 条车厢记录")
        ...
        if seat_records:
            ...
            logger.debug(f"已插入 {len(seat_records)} 条座位记录")
...
def main():
    """主函数：获取车次 -> 生成数据 -> 批量导入 -> 验证"""
    logger.info("=" * 60)
    logger.info("座位和车厢数据生成脚本启动")
    logger.info("=" * 60)
    ...
    try:
        ...
        for train in trains:
            ...
            logger.info(f"处理车次：{train_number} ({train_brand})")

            carriage_records, seat_records = build_records_for_train(train_id, train_brand)

            all_carriage_records.extend(carriage_records)
            all_seat_records.extend(seat_records)
            processed_count += 1

            # 每处理 10 个车次输出一次进度
            if processed_count % 10 == 0:
                logger.info(f"  进度：{processed_count}/{len(trains)} 车次，累计 {len(all_carriage_records)} 车厢，{len(all_seat_records)} 座位")

        # Step 3: 批量插入数据库
        if all_carriage_records:
            save_to_database(all_carriage_records, all_seat_records, processed_count)
            logger.info(f"成功插入 {len(all_carriage_records)} 条车厢记录和 {len(all_seat_records)} 条座位记录")
        else:
