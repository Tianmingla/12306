...
sequenceDiagram
    ...
    participant R as Redis
    participant M as RocketMQ
    participant SR as SeatReleaseConsumer
    participant WF as WaitlistFulfillConsumer
    participant SC as SeatSelectionConsumer
    participant WSRC as WaitlistSeatResultConsumer
    ...
    OS->>OS: 生成waitlistSn
    OS->>OS: 计算优先级
    OS->>R: 入队ZSet(score=priority)
    OS-->>U: 返回waitlistSn

    %% 等待事件触发（退票/新增座位）
    Note over OS,R: 候补订单等待事件触发

    %% 座位释放触发候补
    alt 用户退票
        U->>OS: refundOrder()
        OS->>OS: 释放座位
        OS->>SR: 发送SeatReleaseMessage
    else 新增座位
        Admin->>OS: 新增座位
        OS->>SR: 发送SeatReleaseMessage
    end
    ...
    SR->>R: Lua脚本释放座位
    SR->>M: 发送WaitlistFulfillMessage

    %% 候补兑现
    M->>WF: waitlist-fulfill消息
    WF->>R: ZPOPMAX出队(最高优先级)
    WF->>OS: 查询候补订单(status=0)
    WF->>OS: 更新状态=兑现中(1)
    WF->>M: 发送seat-selection(WAITLIST)

    %% 选座处理
    M->>SC: seat-selection消息
    SC->>SC: 执行选座
    alt 选座失败
        SC->>M: 发送seat-selection-result(success=false)
    else 选座成功
        SC->>M: 发送seat-selection-result(success=true, seats)
    end

    %% 候补结果处理
    M->>WSRC: seat-selection-result消息
    WSRC->>OS: 查询候补订单(status=1)
    alt 选座成功
        WSRC->>M: 发送order-creation(携带waitlistSn)
    else 选座失败
        WSRC->>OS: 状态=待兑现(0)
        WSRC->>R: 惩罚-10分,重新入队ZSet
    end
...
```mermaid
flowchart TD
    Start[用户提交候补订单] --> Queue[加入候补队列<br/>状态=待兑现]

    Queue --> Wait1{等待事件触发}
    Wait1 -->|退票/新增座位| Select[触发候补兑现<br/>ZPOPMAX出队]
    ...
    Result -->|失败| Fail[状态=待兑现<br/>优先级-10分<br/>重新入队]
    
    Success --> End1[完成]
    Fail --> Wait1

    %% 其他终止路径
    Queue --> ExpireCheck{截止时间检查}
    ExpireCheck -->|已过期| Expire[状态=已过期]
    Expire --> End2[结束]
    
    Queue --> UserCancel{用户操作}
    UserCancel -->|主动取消| Cancel[状态=已取消]
    Cancel --> End2
