...
候补订单的完整处理链路涉及多个微服务和MQ Topic，形成一条完整的异步流水线。
...
flowchart TD
    ...
    R1 --> M1[发送 waitlist-check<br/>延迟5秒]
    
    M1 --> C1[WaitlistCheckConsumer]
    C1 --> C2{状态检查}
    C2 -->|异常| C3[跳过]
    C2 -->|正常| C4{截止时间?}
    C4 -->|已过期| C5[状态=4已过期]
    ...
    C6 --> C7{余票检查<br/>Redis缓存}
    C7 -->|有票| C8[发送 seat-selection<br/>WAITLIST来源]
    C7 -->|无票| C9[状态=0待兑现<br/>recalcPriority]
    
    C8 --> C10[SeatSelectionConsumer]
    C10 --> C11{选座结果}
    C11 -->|无票| C12[返回null<br/>继续排队]
    C11 -->|成功| C13[发送结果<br/>seat-selection-result]
    
    C13 --> C14[OrderResultProcessorConsumer]
    C14 --> C15[计算票价]
    C15 --> C16[发送 order-creation]
    
    C16 --> C17[OrderCreationConsumer]
    C17 --> C18[创建订单DB]
    C18 --> C19[发送结果<br/>order-creation-result]
    
    C19 --> C20[WaitlistResultConsumer]
    C20 --> C21{订单结果}
    C21 -->|成功| C22[状态=2已兑现<br/>orderSn记录<br/>ZSet移除]
    C21 -->|失败| C23[状态=0待兑现<br/>惩罚-10分<br/>重新入队]
    ...
    subgraph "Redis"
        ...
        Hash["Hash: WAITLIST:DETAIL::{waitlistSn}<br/>订单详情缓存"]
    end
    
    subgraph "消息队列 Topics"
        T1["waitlist-check-topic<br/>Tag: check"]
        T2["seat-selection-topic<br/>Tag: select"]
        T3["seat-selection-result-topic<br/>Tag: *"]
        T4["order-creation-topic<br/>Tag: create"]
        T5["order-creation-result-topic<br/>Tag: *"]
    end
...
#### 5.5.5.1 WaitlistCheckConsumer（候补检查消费者）

**类路径**：`com.lalal.modules.consumer.WaitlistCheckConsumer`
...
**处理逻辑**：
1. **幂等性保护**：使用 `requestId` 作为Redis SETNX锁，TTL 30分钟
2. **订单校验**：查询候补订单，仅处理状态为"待兑现"(0)的订单
3. **过期检查**：截止时间早于当前时间的订单，状态更新为"已过期"(4)
4. **状态更新**：更新为"兑现中"(1)，表示正在尝试兑现
5. **余票检查**：遍历用户选择的座位类型，查询Redis余票缓存
   - Key格式：`TICKET:REMAINING::{trainNumber}::{travelDate}::{seatType}`
   - 任一类型有票即触发选座
6. **分支处理**：
   - 有票：发送 `SeatSelectionRequestMessage` 到 `seat-selection-topic`，`source="WAITLIST"`
   - 无票：回滚状态为"待兑现"(0)，调用 `recalculatePriority()` 重新计算优先级

**核心代码片段**：
```java
// 余票检查（优先缓存，无缓存保守返回null）
private Integer checkTicketAvailability(WaitlistCheckMessage msg) {
    for (Integer seatType : msg.getSeatTypes()) {
        String cacheKey = String.format(
            ...
            msg.getTrainNumber(), msg.getTravelDate(), seatType
        );
    ...
    return null; // 无缓存数据，保守返回无票
}
```

#### 5.5.5.2 WaitlistResultConsumer（候补结果消费者）

**类路径**：`com.lalal.modules.consumer.WaitlistResultConsumer`
...
**处理逻辑**：
1. **来源识别**：通过 `TicketAsyncRequestDO` 缓存中的 `waitlistSn` 字段识别候补订单
2. **幂等性保护**：检查候补订单状态，终态（2/3/4）直接跳过
3. **成功处理**：
   - 更新候补状态为"已兑现"(2)
   - 记录 `fulfilledOrderSn`（关联的订单号）
   - 调用 `waitlistQueueService.remove()` 从Redis ZSet移除
4. **失败处理**：
   - 更新候补状态为"待兑现"(0)
   - 调用 `waitlistService.recalculatePriorityWithPenalty()` 扣10分
   - 触发重新排队
...
|-------|-----|--------|--------|--------|------|
| `waitlist-check-topic` | `check` | WaitlistService | WaitlistCheckConsumer | `WaitlistCheckMessage` | 候补检查请求 |
| `seat-selection-topic` | `select` | WaitlistCheckConsumer | SeatSelectionConsumer | `SeatSelectionRequestMessage`（source=WAITLIST） | 选座请求 |
| `seat-selection-result-topic` | `*` | SeatSelectionConsumer | OrderResultProcessorConsumer | `SeatSelectionResultMessage` | 选座结果 |
| `order-creation-topic` | `create` | OrderResultProcessorConsumer | OrderCreationConsumer | `OrderCreationRequestMessage` | 订单创建请求 |
| `order-creation-result-topic` | `*` | OrderCreationConsumer | WaitlistResultConsumer<br/>OrderResultProcessorConsumer | `OrderCreationResultMessage` | 订单创建结果 |

**消息定义（Frameworks/common 模块）**：

1. **WaitlistCheckMessage** - 候补检查请求
```java
@Data
public class WaitlistCheckMessage implements Serializable {
    private String requestId;        // 全局请求ID（幂等键）
    private String waitlistSn;       // 候补订单号
    private String trainNumber;      // 车次
    private String travelDate;       // 乘车日期 yyyy-MM-dd
    private List<Integer> seatTypes; // 座位类型列表
    private Integer priority;        // 当前优先级（用于日志）
    private String source;           // 来源，固定"WAITLIST"
...
3. **WaitlistCheckResultMessage** - 检查结果（用于前端轮询）
```java
@Data
public class WaitlistCheckResultMessage implements Serializable {
    private String requestId;
    private String waitlistSn;
    private boolean hasTicket;       // 是否有票
    private Integer availableCount;  // 可用票数
    private String reason;           // 无票原因
